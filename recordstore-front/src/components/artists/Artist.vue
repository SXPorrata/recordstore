    ...
    <ul class="list-reset mt-4">
      <li class="py-4" v-for="artist in artists" :key="artist.id" :artist="artist">
        <div class="flex items-center justify-between flex-wrap">
          ...
          <button class="bg-transparent text-sm hover:bg-blue hover:text-white text-blue border border-blue no_underline font-bold py-2 px-4 mr-2 rounded" @click.prevent="editArtist(artist)">Edit</button>
  ...
  created () {
    if (!localStorage.signedIn) {
      ...
      this.$http.secured.get('/api/v1/artists')
        ...
        .catch(error => this.setError(error, 'Somthing went wrong'))
    }
  },
  methods: {
    setError (error, text) {
    ...
    updateArtist (artist) {
      ...
      this.$http.secured.patch(`/api/v1/artist/${artist.id}`, { artist: { title: artist.name } })
        .catch(error => this.setError(error, 'Cannot update artists'))
    }
  }
}
</script>
